...
public class AtaqueRetrocesoSustancial extends AtaqueOfensivos {
	public AtaqueRetrocesoSustancial(String nombre, String categoria, int potencia, int precision, int pp, boolean prioridad,Tipo.Tipos tipoAtaque) {
	...
	public void utilizar(Pokemon pAtacante, Pokemon pEnemigo) {
		...
		if (tienePps()) {
			...
			if (acierta()) {
				...
				int retroceso = pAtacante.getPuntosVidaMaximo()/3;
				pAtacante.reducirPuntosDeVida(retroceso);
				System.out.println(pAtacante.validarNombre() + " ha usado " + getNombre() + " pero falló...");
				System.out.println(pAtacante.validarNombre() + " ha perdido " +retroceso + " puntos de vida");
			}
		} else {
